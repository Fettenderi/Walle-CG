    ...
    public:
        ...
        void RenderText(std::string text, glm::vec2 coordinates, float scale, glm::vec3 color) {
            ...
            for (c = text.begin(); c != text.end(); c++)
            {
                ...
                float xpos = coordinates.x + ch.Bearing.x * scale;
                float ypos = coordinates.y - (ch.Size.y - ch.Bearing.y) * scale;
                ...
                coordinates.x += (ch.Advance >> 6) * scale; // bitshift by 6 to get value in pixels (2^6 = 64 (divide amount of 1/64th pixels by 64 to get amount of pixels))
            }
            glBindVertexArray(0);
            glBindTexture(GL_TEXTURE_2D, 0);
        }
